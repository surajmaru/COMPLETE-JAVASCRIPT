...
const showRecipe = async function(){
  ...
  const res = await fetch(
    `https://forkify-api.jonas.io/api/v2/recipes/${id}` // And use it here for dynamic api.
  );
  const data = await res.json();

  if(!res.ok) throw new Error(`${data.message} (${res.status})`)

  let {recipe} = data.data; // Creating a new object from the apis object for our better understanding.
  recipe = {
    id: recipe.id,
    title: recipe.title,
